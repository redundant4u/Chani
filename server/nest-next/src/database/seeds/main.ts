...
export class SeedMain implements Seeder {
    public async run(factory: Factory, connection: Connection): Promise<any> {
        ...
        await connection.getRepository(CorpEntity).query(`TRUNCATE TABLE corp CASCADE`);
        ...
        file = readFileSync("./src/database/corp.json", "utf8");
        json = JSON.parse(file);

        for (let i of json) {
            await connection.getRepository(CorpEntity).save({
