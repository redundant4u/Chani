...
class Financial:
    year = 0
    year_key = [ 'thstrm_amount', 'frmtrm_amount', 'bfefrmtrm_amount' ]
    financial_data = {}
    ...
    def add_financial_statement(self, kind, index):
        if( kind == 0 ): self.financial_data['stock_code'] = financial_raw_data_19[index]['stock_code'].zfill(6)
        if( self.year == 2019 ):
            if( kind == 2 ):
                self.financial_data['net_income'] = int(index[ self.year_key[0] ])

                # 비지배 당기순이익 가져오기, 비지배 당기순이익 값이 없을 수도 있으므로 기본 0 값으로 지정
                self.financial_data['net_income_non'] = 0

            elif( kind == 3 ):
                self.financial_data['net_income_non'] = int(index[ self.year_key[0] ])
            ...
            if( kind == 1 ):
                for i in financial_raw_data_18:
                    if( i['stock_code'] == financial_raw_data_18[] )
...
def parse_json():
    ...
    for year in years:
        ...
        for i in range( 0, 1):#len(corp_info) ):
            init_json()
            issued_stock = int(financial_raw_data_19[i]['issued_stock'])
...
def add_stock_code(i):
    financial.add_financial_statement(kind.stock_code, i)

def add_issued_stock(i):
    fincial.add_financial_statement(kind.issued_stock, i)
    financial.financial_data['issued_stock']
...
def add_net_income(financial_tmp):
    ...
    for i in financial_tmp:
        if( find.net_income(i) ):
            try:
                financial.add_financial_statement(kind.net_income, i)
    ...
    for i in financial_tmp:
        if( find.net_income_non(i) ):
            try:
                financial.add_financial_statement(kind.net_income_non, i)
